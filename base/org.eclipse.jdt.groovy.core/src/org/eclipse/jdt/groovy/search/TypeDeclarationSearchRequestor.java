        ...
        if (node instanceof ClassNode) {
            ...
            if (redirect.getNameEnd() > 0 && orig == redirect) {
                if (pattern.matchesName(simpleNamePattern, orig.getNameWithoutPackage().toCharArray())) {
                    ...
                    if (matchFound) {
                        try {
                            IJavaElement realElement = enclosingElement.getOpenable() instanceof GroovyClassFileWorkingCopy
                                ? ((GroovyClassFileWorkingCopy) enclosingElement.getOpenable()).convertToBinary(enclosingElement)
                                : enclosingElement;
