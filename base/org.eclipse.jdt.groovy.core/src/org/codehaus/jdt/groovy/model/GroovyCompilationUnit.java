...
public class GroovyCompilationUnit extends CompilationUnit {
	...
	@SuppressWarnings("unchecked")
	protected boolean buildStructureOLD(OpenableElementInfo info, IProgressMonitor pm, Map newElements, IResource underlyingResource)
			throws JavaModelException {
		...
		// surround in try catch so that compiler failures do not propogate up
		GroovyCompilationUnitDeclaration compilationUnitDeclaration = null;
		try {
			// compiler options
			Map<String, String> options = (project == null ? JavaCore.getOptions() : project.getOptions(true));
			...
			// Create the structure necessary to do a resolve
			...
			INameEnvironment environment = new CancelableNameEnvironment(project, owner, pm);
			// LookupEnvironment lookupEnvironment = new LookupEnvironment(null,
			// compilerOptions, reporter, environment);
			GroovyResolveRequestor resolveRequestor = new GroovyResolveRequestor();

			// do the resolve
			Compiler compiler = new GroovyCompilationUnitProblemFinder(environment,
					new GroovyErrorHandlingPolicy(!computeProblems), compilerOptions, resolveRequestor, new DefaultProblemFactory());

			compilationUnitDeclaration = (GroovyCompilationUnitDeclaration) compiler.resolve(this, true, true, true);
			// move the structure over to this GroovyCompilationUnit
			// ie- convert from compiler AST to JavaElements
			GroovyCompilationUnit source = this.cloneCachingContents();
			CompilationUnitStructureRequestor structureRequestor = new GroovyCompilationUnitStructureRequestor(source, unitInfo,
					newElements);
			SourceElementNotifier notifier = new SourceElementNotifier(structureRequestor, true);

			if (compilationUnitDeclaration != null) {
				// TODO GROOVY this should be calculated in the
				// compilationUnitDeclaration
				HashtableOfObjectToInt sourceEnds = createSourceEnds(compilationUnitDeclaration);
				// TODO what should go in the nodesToCategories map (ie- the last
				// argument)?
				notifier.notifySourceElementRequestor(compilationUnitDeclaration, 0, getContents().length, true, sourceEnds,
						new HashMap());
			}

			// collect the problems
			if (computeProblems && resolveRequestor.getResults().size() > 0) {
				// note that perWorkingCopyInfo is not null here
				calculateProblems(info, unitInfo, perWorkingCopyInfo, resolveRequestor.getResults().get(0));
			}
	...
	private void calculateProblems(OpenableElementInfo info, CompilationUnitElementInfo unitInfo,
			JavaModelManager.PerWorkingCopyInfo perWorkingCopyInfo, CompilationResult result) {
		HashMap<String, CategorizedProblem[]> problems = null;
		if (info instanceof ASTHolderCUInfo) {
			problems = (HashMap<String, CategorizedProblem[]>) ReflectionUtils.getPrivateField(ASTHolderCUInfo.class, "problems",
					info);
		}
		...
		try {
			...
			// problems
			CategorizedProblem[] unitProblems = result.getProblems();
			int length = unitProblems == null ? 0 : unitProblems.length;
			if (length > 0) {
				if (reportProblems) {
					for (CategorizedProblem unitProblem : unitProblems) {
						perWorkingCopyInfo.acceptProblem(unitProblem);
					}
				}
				unitInfo.setIsStructureKnown(false);
				CategorizedProblem[] copiedProblems = new CategorizedProblem[length];
				System.arraycopy(unitProblems, 0, copiedProblems, 0, length);
				problems.put(IJavaModelMarker.JAVA_MODEL_PROBLEM_MARKER, copiedProblems);
			}

			// tasks
			CategorizedProblem[] unitTasks = result.getProblems();
			length = unitTasks == null ? 0 : unitTasks.length;
			if (length > 0) {
				CategorizedProblem[] copiedProblems = new CategorizedProblem[length];
				System.arraycopy(unitTasks, 0, copiedProblems, 0, length);
				problems.put(IJavaModelMarker.TASK_MARKER, copiedProblems);
			}
		} finally {
			perWorkingCopyInfo.endReporting();
		}
	...
	private void handleResolveException(RuntimeException e) throws JavaModelException {
		String lineDelimiter = this.findRecommendedLineSeparator();
