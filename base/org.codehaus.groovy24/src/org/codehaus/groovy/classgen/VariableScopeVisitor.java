...
public class VariableScopeVisitor extends ClassCodeVisitorSupport {
    ...
    private Variable checkVariableNameForDeclaration(String name, Expression expression) {
        ...
        while (true) {
            ...
            Variable var1 = scope.getDeclaredVariable(name);
            if (var1 != null) {
                var = var1;
                break;
            }

            var1 = scope.getReferencedLocalVariable(name);
            if (var1 != null) {
                var = var1;
                break;
            }

            var1 = scope.getReferencedClassVariable(name);
            if (var1 != null) {
                var = var1;
                break;
            }
            ...
            if (classScope != null) {
                ...
                // GRECLIPSE add -- GROOVY-9598
                while (member == null && classScope.getOuterClass() != null && !isAnonymous(classScope)) {
                    crossingStaticContext = crossingStaticContext || isStatic(classScope.getModifiers());
                    member = findClassMember((classScope = classScope.getOuterClass()), name);
                }
                // GRECLIPSE end
                if (member != null) {
