...
public class AnnotationVisitor {
    ...
    private boolean checkIfMandatoryAnnotationValuesPassed(final AnnotationNode node) {
        ...
        for (MethodNode mn : classNode.getMethods()) {
            ...
            */
            if (!mn.hasAnnotationDefault() && !mn.isStaticConstructor() && !node.getMembers().containsKey(methodName) && !"dataVariableNames".equals(methodName)) {
    ...
    protected void visitExpression(final String attrName, final Expression valueExpr, final ClassNode attrType) {
        if (attrType.isArray()) {
            ...
            if (valueExpr instanceof ListExpression) {
                ...
                // GRECLIPSE add
                listExp.setSourcePosition(ClassCodeVisitorSupport.getNonInlinedExpression(valueExpr));
                // GRECLIPSE end
                if (annotation != null) {
