...
public class InnerClassCompletionVisitor extends InnerClassVisitorHelper implements Opcodes {
    ...
    public void visitConstructor(ConstructorNode node) {
        ...
        if (((InnerClassNode) classNode).isAnonymous() && classNode.getOuterClasses().contains(classNode.getSuperClass())) {
            ...
            if (superCtor != null && superCtor.isPrivate()) {
                ...
                if (statement instanceof BlockStatement) statement = ((BlockStatement) statement).getStatements().get(0);
                ConstructorCallExpression superCtorCall = (ConstructorCallExpression) ((ExpressionStatement) statement).getExpression();
                ((TupleExpression) superCtorCall.getArguments()).addExpression(new CastExpression(superClass, ConstantExpression.NULL));
            }
        }
        // GRECLIPSE end
    }
    ...
    private static void makeBridgeConstructor(final ClassNode c, final Parameter[] p) {
        ...
        if (c.getDeclaredConstructor(newP) == null) {
            ...
            addGeneratedConstructor(c, ACC_SYNTHETIC, newP, ClassNode.EMPTY_ARRAY, new ExpressionStatement(ctorThisX(args)));
        }
    }
    // GRECLIPSE end
