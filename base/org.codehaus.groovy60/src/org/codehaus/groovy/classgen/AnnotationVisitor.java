    ...
    protected void visitExpression(final String attrName, final Expression valueExpr, final ClassNode attrType) {
        if (attrType.isArray()) {
            ...
            if (valueExpr instanceof ListExpression listExpr) {
                ...
                // GRECLIPSE add
                listExpr.setSourcePosition(ClassCodeVisitorSupport.getNonInlinedExpression(valueExpr));
                // GRECLIPSE end
                if (annotation != null) {
            ...
            if (!(valueExpr instanceof ClassExpression || valueExpr instanceof ClosureExpression)) {
                addError("Only classes and closures can be used for attribute '" + attrName + "'", valueExpr); // GRECLIPSE
            }
