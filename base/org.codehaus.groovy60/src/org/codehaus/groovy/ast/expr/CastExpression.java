...
public class CastExpression extends Expression {
    ...
    // GRECLIPSE add
    public void forEachType(final java.util.function.Consumer<ClassNode> sink) {
        ClassNode type = getType();
        if (type instanceof IntersectionTypeClassNode it) {
            for (ClassNode cn : it.getComponents()) {
                sink.accept(cn);
            }
        } else {
            sink.accept(type);
