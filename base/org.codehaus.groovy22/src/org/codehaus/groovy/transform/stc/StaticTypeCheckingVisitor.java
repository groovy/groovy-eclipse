...
*/
    ...
    private List<MethodNode> disambiguateMethods(List<MethodNode> methods, final Expression expr) {
        if (methods.size()>1) {
            // GRECLIPSE add
            List<MethodNode> filtered = new LinkedList<MethodNode>();
            for (MethodNode methodNode : methods) {
                if ((methodNode.getModifiers() & /*bridge:*/0x40) == 0) {
                    filtered.add(methodNode);
                }
            }
            if (filtered.size()==1) {
                return filtered;
            ...
            // GRECLIPSE end
            methods = extension.handleAmbiguousMethods(filtered, expr);
        }
        return methods;
    }
     ...
     * @param placeholders generics placeholders from context
