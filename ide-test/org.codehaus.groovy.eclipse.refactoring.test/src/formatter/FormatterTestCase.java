...
public class FormatterTestCase extends BaseTestCase {
	...
	private void doTest() {
		...
		DefaultGroovyFormatter formatter = new DefaultGroovyFormatter(selection,
				getDocumentProvider(), pref, indentendOnly);
		try {
			formatter.format().apply(getDocumentProvider());
    ...
    public void finalAssert() {
		...
		String content = getDocumentProvider().get();
