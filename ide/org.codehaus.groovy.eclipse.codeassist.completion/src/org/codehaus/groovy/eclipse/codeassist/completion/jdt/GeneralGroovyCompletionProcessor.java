...
/**
 * Code completion processor that attempts to complete expressions in the source
 * code.
 * 
 * When not in script or closure, then JDT takes over some kinds of completions,
 * such as methods, and Types (not done are categories and inference)
 * 
 * @author empovazan
 * @author andrew
 */
public class GeneralGroovyCompletionProcessor extends
    ...
    public GeneralGroovyCompletionProcessor() {
        IPreferenceStore preferenceStore = JavaPlugin.getDefault()
                .getPreferenceStore();
        isGuessArguments = preferenceStore
    ...
    public List<ICompletionProposal> computeCompletionProposals(
        ...
        try {
            SourceContextInfo info = SourceContextInfo.create(moduleNode,
                    new GroovyProjectFacade(javaContext.getProject()), offset,
                    buffer, true);

            if (info != null) {
                if (info.name != null) {
                    // Complete the property expression.
                    return createCompletionProposals(info, offset, javaContext,
                            monitor);
                }
            }
        } catch (Exception e) {
    ...
    private List<ICompletionProposal> createCompletionProposals(SourceContextInfo info,
            int actualCompletionPosition, JavaContentAssistInvocationContext javaContext,
            IProgressMonitor monitor) {
        List<ICompletionProposal> proposals = new ArrayList<ICompletionProposal>();

        EvalResult result = info.eval;
        String name = info.name;
        String expression = info.expression;
        IMemberLookup lookup = info.lookup;
        boolean dotIsIncluded = info.dotIsIncluded;
        List<ICompletionProposal> results = new ArrayList<ICompletionProposal>();

        // expression is null when not in a place that makes sense for
        // method, field, or property proposals
        if (isValidForMethodProposals(info)) {
            String type = box(result.getName());
    
            Property[] properties = lookup.lookupProperties(type, name, false,
                    result.isClass(), false);
            properties = (Property[]) mergeTypes(properties);
            proposals.addAll(createCompletionProposals(properties,
                    actualCompletionPosition - name.length(), name.length(),
                    javaContext));
    
            Field[] fields = lookup.lookupFields(type, name, false, result
                    .isClass(), false);
            fields = (Field[]) mergeTypes(fields);
            proposals.addAll(createCompletionProposals(fields,
                    actualCompletionPosition - name.length(), name.length(),
                    javaContext));
    
            Method[] methods = lookup.lookupMethods(type, name, false, result
                    .isClass(), false);
            methods = (Method[]) mergeTypes(methods);
            proposals.addAll(createCompletionProposals(methods,
                    actualCompletionPosition - name.length(), name.length(),
                    javaContext));
        ...
        // now look for types, but only if this is not part of a dotted expression
        if (! dotIsIncluded) {
            try {
                SearchableEnvironment nameEnvironment = ((JavaProject) javaContext
                        .getProject()).newSearchableNameEnvironment(javaContext
                                .getCompilationUnit().getOwner());
                String toSearch = name.startsWith("new ") ? name.substring(4) : name;
                GroovyProposalTypeSearchRequestor requestor = new GroovyProposalTypeSearchRequestor(
                        actualCompletionPosition - toSearch.length(), toSearch.length(), actualCompletionPosition,
                        javaContext, nameEnvironment.nameLookup, monitor);
                
                nameEnvironment.findTypes(toSearch.toCharArray(), true, // all member
                                                                    // types, should
                                                                    // be false when
                                                                    // in
                                                                    // constructor
                        true, // camel case match
                        IJavaSearchConstants.TYPE, requestor, monitor);
    
                List<ICompletionProposal> typeProposals = requestor
                        .processAcceptedTypes();
                results.addAll(typeProposals);
            } catch (JavaModelException e) {
                GroovyCore.logException(
                        "Exception when looking up types for content assist", e);
            }
        }
     ...
     * @return
     */
    private boolean isValidForMethodProposals(SourceContextInfo info) {
        return info.expression != null && info.eval != null && !info.eval.getName().equals("void");
    }

    /**
     * If type is a primitive type name then boxes the type in with the object
     * name Otherwise, just return type.
     * 
     * @param type
     */
    ...
    private List<ICompletionProposal> createCompletionProposals(
            Member[] members, int offset, int replaceLength,
        ...
        for (int i = 0; i < members.length; ++i) {
            ...
            if (replaceString.indexOf('$') == -1
                    && !replaceString.startsWith("__timeStamp")) {
                proposals.add(new JavaCompletionProposal(replaceString, offset,
                        replaceLength, getImageForType(members[i]),
                        createDisplayString(members[i]),
                        getRelevance(members[i].getName().toCharArray())));
            }
        }
        return proposals;
    }
    ...
    private int getRelevance(char[] name) {
        return name[0] == '$' || name[0] == '_' ? 1 : 20;
    }

    private List<ICompletionProposal> createCompletionProposals(
            Method[] methods, int offset, int replaceLength,
            JavaContentAssistInvocationContext javaContext) {
        List<ICompletionProposal> proposals = new ArrayList<ICompletionProposal>(
                methods.length);
        for (int i = 0; i < methods.length; ++i) {
            ...
            if (replaceString.indexOf('$') == -1) {
                CompletionProposal proposal = CompletionProposal.create(
                        CompletionProposal.METHOD_REF, offset + replaceLength);
                proposal.setCompletion(replaceString.toCharArray());
                proposal.setDeclarationSignature(methods[i].getDeclaringClass()
                        .getSignature().toCharArray());
                proposal.setName(methods[i].getName().toCharArray());
                proposal
                        .setParameterNames(createParameterNames((Method) methods[i]));
                ReflectionUtils.setPrivateField(
                        InternalCompletionProposal.class, "parameterTypeNames",
                        proposal, createParameterTypeNames(methods[i]));
                ...
                proposal.setSignature(getMethodSignature(methods[i])); // this
                                                                       // one
                                                                       // might
                                                                       // be a
                                                                       // real
                                                                       // type
                                                                       // signature
                proposal.setRelevance(getRelevance(proposal.getName())); // should
                                                                         // set
                                                                         // lower
    ...
    private char[][] createParameterTypeNames(Method method) {
        ...
        for (Parameter param : method.getParameters()) {
            ...
            if (Signature.getArrayCount(param.getSignature()) > 0) {
                typeNames[i] = Signature.getSignatureSimpleName(param
                        .getSignature().toCharArray());
            } else {
                // this is a plain old qualified name
                String sig = param.getSignature();
                String[] splits = sig.split("\\.");
                typeNames[i] = (splits.length > 0 ? splits[splits.length - 1]
                        : sig).toCharArray();
    ...
    private char[] getMethodSignature(Method method) {
        ...
        return Signature.createMethodSignature(paramTypeSigs, returnTypeSig)
                .toCharArray();
    }
