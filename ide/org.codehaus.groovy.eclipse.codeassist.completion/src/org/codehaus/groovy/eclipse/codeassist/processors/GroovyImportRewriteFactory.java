...
import org.codehaus.groovy.eclipse.codeassist.processors.GroovyProposalTypeSearchRequestor.CharArraySequence;
...
public class GroovyImportRewriteFactory {
	...
	public ImportRewrite getImportRewrite(IProgressMonitor monitor) {
		...
		// When invoked in organise imports, it is important to check for
		// unrecoverable errors. The reason is that if there are no
		// unrecoverable errors, then a standard import rewrite should be used
		// instead of the one generated by this method. However in the
		// case of a individual add import quick fix, which also contains no
		// unresolved errors if only the unresolved type is the error,
		// this special import rewrite should be used, therefore for add import
		// quick fix, it is not necessary to pass a module to check for
