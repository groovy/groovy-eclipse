...
private void initAttributes(IJavaElement javaElement) {
	if (javaElement != null) {
		try {
			// GROOVY add
			var classFile = (IClassFile) javaElement.getAncestor(IJavaElement.CLASS_FILE);
			if (classFile != null) {
				var primaryType = (BinaryType) classFile.findPrimaryType();
				if (primaryType != null) {
					IBinaryType elementInfo = primaryType.getElementInfo();
